...
function constrainFireBallInsideCanvas() {
    if(fireBall.x < -100 || fireBall.x > 2100) { //if it goes outside the canvas to the right or the left
        fireBallDeath();
    } else if (fireBall.y < -100 || fireBall.y > 2100) { //if it goes outside the canvas up or down
        fireBallDeath();
    }
}
...
function fireBallDeath() {
    ...
    fireBall.shooting.direction = "none"
    fireBall.y = fireBall.originalY;
    fireBall.x = fireBall.originalX;
    fireBall.shooting.speed = 5
    fireBall.speed = 7
}
